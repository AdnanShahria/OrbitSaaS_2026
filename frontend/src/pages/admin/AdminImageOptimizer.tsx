...
export default function AdminImageOptimizer() {
    ...
    useEffect(() => {
        if (hasLoadedImages && discoveredImages.length > 0 && Object.keys(imageSizes).length === 0 && !fetchingSizes) {
            fetchSizes();
        }
    }, [discoveredImages, hasLoadedImages, fetchingSizes]); // eslint-disable-line react-hooks/exhaustive-deps
    ...
    return (
        <div className="space-y-8 relative">
            ...
            {!hasLoadedImages ? (
                <div className="relative overflow-hidden bg-gradient-to-br from-blue-500/5 via-secondary/20 to-purple-500/5 border border-border/80 rounded-2xl p-8 md:p-12 text-center shadow-xl backdrop-blur-md">
                    ...
                    <div className="max-w-md mx-auto space-y-6 relative z-10">
                        <div className="inline-flex items-center justify-center w-20 h-20 rounded-full bg-blue-500/10 border border-blue-500/20 shadow-inner">
                        ...
                        <div className="space-y-2">
                            ...
                            <p className="text-sm text-muted-foreground leading-relaxed">
                                We discovered <span className="text-blue-500 font-semibold">{discoveredImages.length} images</span> in your site content. Click below to load their details and preview thumbnails.
                            </p>
                        </div>
                        <button
                            onClick={() => setHasLoadedImages(true)}
                            className="inline-flex items-center justify-center px-8 py-3.5 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-semibold rounded-xl shadow-lg shadow-blue-500/20 hover:shadow-xl hover:shadow-blue-500/30 transition-all duration-300 hover:scale-[1.02] active:scale-[0.98] cursor-pointer"
                        >
                            <Zap className="w-4 h-4 mr-2" /> Load Site Images
                        </button>
                    </div>
                </div>
            ) : (
